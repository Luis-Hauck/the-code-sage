...
from src.utils.embeds import UserEmbeds,InventoryEmbeds, create_error_embed, create_info_embed

logger = logging.getLogger(__name__)


class UserCog(commands.Cog):
    """Cog com comandos de perfil, inventário e gerenciamento de equipamento."""
    ...
    async def view_profile(self, interaction: discord.Interaction):
        ...
        if not profile_data:
            await interaction.followup.send(embed=create_error_embed("Erro", "Perfil não encontrado."), ephemeral=True)
            return
    ...
    async def equip_autocomplete(self, interaction: discord.Interaction, current: str) -> List[
        ...
        """Sugere itens do inventário para o comando de equipar.

        Args:
            interaction (discord.Interaction): Interação atual do usuário.
            current (str): Texto parcial digitado no campo do item.

        Returns:
            List[app_commands.Choice[str]]: Lista de sugestões limitadas a 25 itens.
        """
    ...
    async def equip_item(self, interaction: discord.Interaction, item: int):
        """Equipe um item do inventário do usuário.
    ...
    async def unequip_item(self, interaction: discord.Interaction):
        ...
        if sucess:
            embed = create_info_embed(title='Item desequipado com sucesso!',
                                  message='Bora juntar mais moedas para comprar mais um ;)')
    ...
    async def view_inventory(self, interaction: discord.Interaction):
        """Exibe os itens do inventário e o item equipado do usuário.
        ...
        username, equipped_item_name, items = await self.user_service.get_user_inventory(user_id)
        ...
        embed = InventoryEmbeds.view_inventory(user_name=username,
