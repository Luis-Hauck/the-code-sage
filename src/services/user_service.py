...
class UserService:
    """
    Serviço responsável pela lógica de relacionada a usuários.
    """

    def __init__(self,
                 user_repo: UserRepository,
                 item_repo: ItemRepository = None,
                 leveling_service: LevelingService = None):
        """
    ...
    async def sync_guild_users(self, members_data: List[Dict[str, Any]]) -> Tuple[int, int]:
        ...
        new_users, ignored_count = await asyncio.to_thread(_process_users_sync, members_data, existing_ids)
...
def _process_users_sync(members_data: List[Dict], existing_ids: Set[int]) -> Tuple[List[UserModel], int]:
    """
    ...
    for member in members_data:
        ...
        joined_at = member.get('joined_at') or datetime.now()
        new_user = UserModel(
            _id=user_id,
